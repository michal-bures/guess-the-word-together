...
export class GameDirector {
    ...
    startNewGame(roomId: string, secretWord: string, category: string): BackendGameSessionState {
        console.log('Starting new game in room:', roomId, 'with secret word:', secretWord, 'and category:', category);
    ...
    updateQuestion(roomId: string, questionId: string, questionData: Partial<QuestionAnswerPair>): void {
        const session = this.sessions.get(roomId);
